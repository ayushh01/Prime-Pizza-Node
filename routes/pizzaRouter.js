...
pizzaRouter.route("/:pizzaId").get(ensureAuthenticated, (req, res, next) => {
  Pizzas.findById(req.params.pizzaId)
    .then(
      (pizza) => {
        Pizzas.find({ publisher: pizza.publisher }).then((pizz) => {
          if (pizz.length > 3) {
            res.render("selectedpizza", {
              name: req.user.name,
